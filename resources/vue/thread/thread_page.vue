<template>
  <div>
    <div class="thread_body" v-show="posts_load_status == 2 && !thread_response_code">
      <div class="row align-items-center mt-3">
        ...
        <b-dropdown id="shield-dropdown" text="屏蔽" variant="outline-dark" size="sm">
          ...
          <b-form-checkbox v-model="no_custom_emoji_mode" switch class="ml-2 my-2">
            自定义表情包
          </b-form-checkbox>
      ...
      <div class="post_container">
        ...
        <div class="post_title px-1 py-2 h5 d-none d-lg-block d-xl-block">
          <span style="word-wrap: break-word; white-space: normal">标题：{{ thread_title }}</span><span> [{{ thread_posts_num
          }}]</span>
        </div>
        ...
        <CrowdComponent v-if="crowd_id && posts_load_status" :crowd_id="crowd_id" :admin_button_show="admin_button_show">
        </CrowdComponent>
        ...
        <HongbaoPoolComponent v-if="thread_id == 100658" :thread_id="thread_id" :forum_id="forum_id">
        ...
        <div v-for="post_data in posts_data" :key="post_data.id">
          <PostItem :post_data="post_data" :thread_anti_jingfen="thread_anti_jingfen" :random_head_add="random_heads_data[random_heads_group - 1].random_heads[
            ...
            <template v-slot:battle v-if="post_data.battle_id && posts_load_status == 2 && no_battle_mode == false
              ">
            ...
            <template v-slot:hongbao v-if="post_data.hongbao_data && posts_load_status == 2">
              <HongbaoPostComponent ref="hongbao_post_component" :hongbao_data="post_data.hongbao_data"
        ...
        <div class="d-flex flex-row align-items-center my-2">
          <b-button :variant="button_theme" size="sm" id="listen_button" @click="listen_channel"
            :disabled="!enable_listening || show_listen_next_page">{{ !is_listening || show_listen_next_page ? "自动涮锅" :
              "正在涮锅" }}
          </b-button>
      ...
      <PostInput ref="post_input_com" :input_disable="!this.$store.state.User.LoginStatus || Boolean(locked_TTL) || new_post_handling
        ...
        :thread_id="thread_id" @content_commit="new_post_handle"><template v-slot:svg_icon>
          <svg width="28" height="28" viewBox="0.00 0.00 100.00 100.00" xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink" @click="modal_toggle('hongbao_modal')" class="icon-battle">
            <path
              ...
              fill="none" stroke="rgb(68, 68, 68)" stroke-width="4.79" stroke-opacity="1.00" stroke-linejoin="round" />
            <path
              ...
              fill="none" stroke="rgb(68, 68, 68)" stroke-width="4.79" stroke-opacity="1.00" stroke-linejoin="miter" />
            <path
            ...
            <path
              ...
              fill="none" stroke="rgb(68, 68, 68)" stroke-width="5.00" stroke-opacity="1.00" stroke-linejoin="round" />
          </svg>
      ...
      <div class="row align-items-center mt-2">
        <div class="col-auto ml-auto" style="font-size: 0.875rem">
          ...
          <span v-if="this.$store.state.Forums.CurrentForumData.is_nissin == 2 &&
            this.$store.state.Threads.CurrentThreadData.sub_id == 0
            ">本贴将于
            <span style="color: #dd0000">{{ nissin_TTL }}</span>
            后日清，请及时更换帖子喔
          </span>
          <span v-if="this.$store.state.Forums.CurrentForumData.is_nissin == 1 &&
            this.$store.state.Threads.CurrentThreadData.sub_id == 0
            ">本小岛定期
    ...
    <img src="https://oss.cpttmm.com/xhg_other/notice_4.png"
      v-if="posts_load_status == 2 && thread_response_code == 21404" class="nissined_img" />
        ...
        <template v-slot:top>
          <div class="icon-roll" @click="modal_toggle('roll_modal')" key="icon-roll">
            <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="dice"
              class="svg-inline--fa fa-dice fa-w-20" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
...
export default {
  ...
  watch: {
    ...
    is_listening() {
      if (this.is_listening === true) {
        try {
          this.$echo.channel("thread_" + this.thread_id).listen("NewPost", (response) => {
            if (response.post_floor >= this.page * 200) {
              ...
              //否则，请求新回复数据
              var post_exist = this.$store.state.Posts.PostsData.data.find((post) => {
                return post.id == response.post_id;
              });
              if (!post_exist) {
                //如果post_id不存在，才去获取新数据
                this.get_post_data_and_push(response.thread_id, response.post_id);
              }
            }
          });
        } catch (e) {
    ...
    no_custom_emoji_mode() {
      localStorage.setItem(
        "no_custom_emoji_mode",
        this.no_custom_emoji_mode ? "true" : ""
      );
    },
    no_head_mode() {
  ...
  computed: {
    nissin_TTL() {
      const seconds =
        (Date.parse(
          this.$store.state.Threads.CurrentThreadData.nissin_date.replace(/-/g, "/") +
          " GMT+800"
        ) -
          Date.now()) /
        1000;
    ...
    ...mapState({
      ...
      thread_anti_jingfen: (state) => state.Threads.CurrentThreadData.anti_jingfen,
      ...
      vote_question_id: (state) => state.Threads.CurrentThreadData.vote_question_id,
      gamble_question_id: (state) => state.Threads.CurrentThreadData.gamble_question_id,
      crowd_id: (state) => state.Threads.CurrentThreadData.crowd_id,
      hongbao_id: (state) => state.Threads.CurrentThreadData.hongbao_id,
      random_heads_group: (state) => state.Threads.CurrentThreadData.random_heads_group,
  ...
  methods: {
    get_posts_data(remind = false, scroll_enable = false, search_content = null) {
      if (search_content) {
        this.$store.commit("PostsLoadStatus_set", 1);
      }
      var config = {
      ...
      axios(config)
        .then((response) => {
          if (response.data.code == 200) {
            ...
            this.$store.commit("CurrentThreadData_set", response.data.thread_data);
            ...
            if ([21404, 23410, 23401, 234011, 23404].includes(response.data.code)) {
    ...
    get_browse_current() {
      if (
        ...
        this.browse_current = JSON.parse(
          JSON.stringify(this.$store.state.User.BrowseLogger[this.thread_id.toString()])
        );
      }
    ...
    thread_delete_click_admin() {
      var content = prompt("要用管理员权限删除这个主题吗？请输入理由", "请输入理由");
      if (content != null) {
        const config = {
          method: "post",
    ...
    check_jingfen_admin() {
      alert("现在不需要这样喔~~");
      return; //暂时关闭此功能
      var content = prompt("要用查看这个主题的反精分吗？请输入理由", "请输入理由");
      const config = {
        method: "post",
      ...
      const config = {
        ...
        data: {
          ...
          new_post_key: CryptoJS.MD5(
            this.thread_id + this.$store.state.User.Binggan + timestamp + content.ist
          ).toString(),
          timestamp: timestamp,
        },
      };
    ...
    jump_handle() {
      ...
      if (this.jump_page) {
        ...
        const link = "/thread/" + this.thread_id + "/" + page + "#f_" + this.jump_floor;
        ...
        .then((response) => {
          if (response.data.code == 200) {
            ...
            if (response.data.data.my_emoji != null) {
              this.$store.commit("MyEmoji_set", JSON.parse(response.data.data.my_emoji));
            }
          } else {
            alert(response.data.message);
          }

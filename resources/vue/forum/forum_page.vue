<template>
  <div>
    <b-carousel id="carousel-fade" fade :interval="10000" img-width="825" img-height="224"
      v-if="threads_load_status == 2 && forum_banners" v-show="!banner_hiden">
      <b-carousel-slide v-for="(banner, index) in forum_banners" :key="index" :img-src="banner"></b-carousel-slide>
    </b-carousel>
    <div class="row align-items-center mt-3">
      ...
      <div class="col-auto d-inline-flex">
        <b-dropdown id="shield-dropdown" text="筛选" variant="outline-dark" size="sm">
          <b-form-checkbox v-for="(option, index) in subtitles_options" :key="index"
            v-model="subtitles_selected[option.value]" @change="set_subtitltes_selected" class="ml-2 my-2">
            {{ option.value }}
          </b-form-checkbox>
        </b-dropdown>
      </div>
      <div class="col-auto my-2 ml-auto d-flex">
        ...
        <b-button size="sm" class="my-1 ml-1 my-sm-0 d-lg-none" variant="outline-dark"
          :disabled="!this.$store.state.User.LoginStatus" @click="search_show = !search_show">搜索</b-button>
        <b-button size="md" class="my-1 ml-1 my-sm-0 d-none d-lg-block" variant="outline-dark"
          :disabled="!this.$store.state.User.LoginStatus" @click="search_show = !search_show">搜索</b-button>
        <b-button size="sm" class="my-1 ml-1 my-sm-0 d-lg-none" :variant="button_theme"
          :disabled="!this.$store.state.User.LoginStatus" @click="$router.push('/loudspeaker')">发大喇叭</b-button>
        <b-button size="md" class="my-1 ml-1 my-sm-0 d-none d-lg-block" :variant="button_theme"
          :disabled="!this.$store.state.User.LoginStatus" @click="$router.push('/loudspeaker')">发大喇叭</b-button>
        <b-button size="sm" class="my-1 ml-1 my-sm-0 d-lg-none" :variant="button_theme"
          :disabled="!this.$store.state.User.LoginStatus" @click="new_thread_botton">发表主题</b-button>
        <b-button size="md" class="my-1 ml-1 my-sm-0 d-none d-lg-block" :variant="button_theme"
          :disabled="!this.$store.state.User.LoginStatus" @click="new_thread_botton">发表主题</b-button>
      </div>
    </div>
    <div class="d-flex flex-row my-2" v-if="search_show">
      <b-form-input id="search_input" class="search_input" style="max-width: 400px" placeholder="目前只支持搜索标题"
        v-model="search_input"></b-form-input>
      <b-button size="sm" class="ml-1" style="min-width: 46px" :variant="button_theme" @click="
        get_threads_data(true, threads_per_page, subtitles_excluded, search_input)
        ">搜索</b-button>
    </div>
    <Loudspeaker></Loudspeaker>
    <ForumThreads :forum_id="forum_id" :new_window_to_post="new_window_to_post" :subtitles_selected="subtitles_selected">
    </ForumThreads>
    <ForumThreadsMobile :forum_id="forum_id" :new_window_to_post="new_window_to_post"
      :subtitles_selected="subtitles_selected"></ForumThreadsMobile>
    <div class="d-flex align-items-center">
      <ForumPaginator :forum_id="forum_id" :last_page="threads_last_page"></ForumPaginator>
      <a v-if="show_delay" @click="get_threads_data" class="thread_title ml-auto">关闭</a>
      <a v-if="!show_delay" @click="get_delay_threads_data" class="thread_title ml-auto">查看延时发送主题</a>
    </div>

    <img src="https://oss.cpttmm.com/xhg_other/notice_404.png"
      v-if="threads_load_status == 2 && thread_reject_code == 22404" class="nissined_img" />

    <img src="https://oss.cpttmm.com/xhg_other/notice_4.png"
      v-if="threads_load_status == 2 && thread_reject_code == 21404" class="nissined_img" />
...
export default {
  components: { ForumThreads, ForumPaginator, ForumThreadsMobile, ZBar, Loudspeaker },
  props: {
